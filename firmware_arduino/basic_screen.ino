...
void draw_basic()
{
  ...
  int temperature_degC = CURRENT_RECORD.EGT[0]; //no average yet, so just use the first value
  ...
  GD.cmd_number(pos_x, pos_y, 31, OPT_CENTERY, CURRENT_RECORD.A0[0]);

  pos_y += 64;
  GET_STRING(A1_VALUE);
  GD.cmd_text(pos_x, pos_y+28, 28, OPT_CENTERY, string);
  GD.cmd_number(pos_x, pos_y, 31, OPT_CENTERY, CURRENT_RECORD.A1[0]);

  pos_y += 64;
  GET_STRING(A2_VALUE);
  GD.cmd_text(pos_x, pos_y+28, 28, OPT_CENTERY, string);
  GD.cmd_number(pos_x, pos_y, 31, OPT_CENTERY, CURRENT_RECORD.A2[0]);

  pos_y += 64;
  GET_STRING(A3_VALUE);
  GD.cmd_text(pos_x, pos_y+28, 28, OPT_CENTERY, string);
  GD.cmd_number(pos_x, pos_y, 31, OPT_CENTERY, CURRENT_RECORD.A3[0]);
