...
void screen_draw_pid_rpm()
{
  ...
  if(sys_mode!=MODE_DIRECT || flags_status.hold_direct_input)
  {
    int rpm_control_target;
    if(sys_mode==MODE_DIRECT)
    {
    ...
    draw_readout_int(GRID_XR(gx,XN), GRID_YC(gy++,YN), DRO_OPT, rpm_control_target, S_TARGET);
  }
  else
  {
