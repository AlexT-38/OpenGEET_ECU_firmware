...
#define PID_FL_TO_FP(f_val)      ((int)((f_val)*(float)PID_FP_ONE))

#define PID_FP_FRAC_BITS        9     //use 7.9 fixed point integers for pid calculations
#define PID_FP_WHOLE_BITS       6     //not including sign bit
#define PID_FP_ONE              (1<<PID_FP_FRAC_BITS)
#define PID_FP_WHOLE_MAX        ((1<<PID_FP_WHOLE_BITS)-1)
#define PID_FP_FRAC_MAX         (PID_FP_ONE-1)
#define PID_FP_MAX              ((PID_FP_ONE*PID_FP_WHOLE_MAX) + PID_FP_FRAC_MAX)

#define PID_OUTPUT_BITS         10
#define PID_OUTPUT_MAX          (_BV(PID_OUTPUT_BITS)-1)
#define PID_OUTPUT_MIN          (0)
#define PID_OUTPUT_CENTRE       ( (PID_OUTPUT_MAX + PID_OUTPUT_MIN) >> 1 )

#define PID_RPM_MIN_FB_RPM      120
