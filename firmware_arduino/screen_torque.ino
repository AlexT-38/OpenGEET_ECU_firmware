...
void screen_draw_torque()
{
  ...
  draw_readout_int(GRID_XR(gx,XN), GRID_YC(gy++,YN), DRO_OPT, torque_LSB, S_TORQUE_LSB);
  ...
  draw_button(GRID_XL(gx,XN), GRID_YT(gy++,YN), GRID_SX(XN),GRID_SY(YN), TAG_CAL_TORQUE_ZERO, S_TQ_SET_ZR_str);
  MAKE_STRING(S_TQ_SET_MX);
  draw_button(GRID_XL(gx,XN), GRID_YT(gy++,YN), GRID_SX(XN),GRID_SY(YN), TAG_CAL_TORQUE_MAX, S_TQ_SET_MX_str);
  MAKE_STRING(S_TQ_SET_MN);
  draw_button(GRID_XL(gx,XN), GRID_YT(gy++,YN), GRID_SX(XN),GRID_SY(YN), TAG_CAL_TORQUE_MIN, S_TQ_SET_MN_str);
